...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch, temperature, calculate_entropy=False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # 如果当前进程分到的 batch 为空，直接返回零，不进入后续复杂的计算逻辑
        if batch_size == 0:
            device = input_ids.device
            # 构造形状为 (0, response_length) 的空 Tensor 保持接口一致
            log_probs = torch.zeros((0, response_length), device=device, dtype=torch.bfloat16)
            entropy = torch.zeros((0, response_length), device=device, dtype=torch.bfloat16) if calculate_entropy else None
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        ...
        for micro_batch in micro_batches:
            ...
            with torch.no_grad():
                if data.batch['input_ids'].shape[0] == 0 or data.batch['input_ids'].numel() == 0:
                    # 如果当前 Worker 没分到有效数据，直接返回空的 log_probs，避免进入 GPU Kernel
                    device = self.actor_module.device
                    # 返回与输入样本数对应的全 0 张量（或者根据 verl 的 DataProto 要求构造空返回）
