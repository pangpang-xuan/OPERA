            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        if not self.async_rollout_mode: # local infer
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        # compute reward model score grpo dont use it
                        if self.use_rm:
                            reward_tensor = self.rm_wg.compute_rm_score(batch)
                            batch = batch.union(reward_tensor)

                        extra_info_list = prepare_extra_info_for_batch(
                                batch=batch,
                        ...
                        if self.config.reward_model.launch_reward_fn_async:
                            ...
                            reward_tensor, reward_extra_infos_dict = compute_reward(batch, self.reward_fn) # reward_extra_info

                    # recompute old_log_probs
                    with marked_timer("old_log_prob", timing_raw, color="blue"):
                        if batch.batch["input_ids"].size(0) > 0:
                            ...
                            print(f"Rank {torch.distributed.get_rank()} received empty batch, skipping compute_log_prob")
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        def to_tensor(data_list, reference_tensor):
                            return torch.tensor(data_list, device=reference_tensor.device, dtype=reference_tensor.dtype)

                        if 'ppl' in reward_extra_infos_dict:
                            ppl_scores = to_tensor(reward_extra_infos_dict['ppl'], reward_tensor)
                            if ppl_scores.dim() == 1:
                                ppl_scores = ppl_scores.unsqueeze(-1)
                            batch.batch["token_level_scores_ppl"] = ppl_scores
                            metrics.update({"critic/score/ppl_mean": ppl_scores.mean().item()})

                        if 'reflection' in reward_extra_infos_dict:
                            refl_scores = to_tensor(reward_extra_infos_dict['reflection'], reward_tensor)
                            if refl_scores.dim() == 1:
                                refl_scores = refl_scores.unsqueeze(-1)
                            batch.batch["token_level_scores_refl"] = refl_scores
                            metrics.update({"critic/score/reflection_mean": refl_scores.mean().item()})

                        total_reward_sum = reward_tensor.sum().item()
                        current_batch_size = reward_tensor.size(0)
                        actual_total_mean = total_reward_sum / current_batch_size
                        metrics.update({
                            "critic/score/total_mean": actual_total_mean,
                        })
                    ...
                    # update critic grpo dont update critic
                    if self.use_critic:
