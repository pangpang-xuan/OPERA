...
import torch
import argparse
from transformers import AutoConfig, AutoModelForCausalLM, AutoModelForTokenClassification, AutoModelForVision2Seq, AutoTokenizer, AutoProcessor
...
if __name__ == '__main__':
    ...
    local_dir = str(root / "actor")
    print("Start converting weights in {}.".format(local_dir))
    output_path = os.path.join(local_dir, 'huggingface_format')
    ...
    for filename in os.listdir(local_dir):
        ...
        if match:
            ...
            break  
    if world_size == 0:
        print("No model file with the proper format")
    ...
    # hf_path = os.path.join(local_dir, 'huggingface_format')
    ...
    if 'ForTokenClassification' in config.architectures[0]:
        ...
        tokenizer = AutoProcessor.from_pretrained(hf_path)
    else:
